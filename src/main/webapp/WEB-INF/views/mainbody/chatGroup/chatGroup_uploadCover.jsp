...
<title><%=application.getServletContextName()%>——上传群组头像</title>
...
<script type="text/javascript">
	$(function(){
		var config = {
				...
				"url" : "${pageContext.request.contextPath}/chatGroup/uploadCover.htm;jsessionid=<%=session.getId()%>",
				"multi" : false,
				"num" : 1,
				"size" : 204800,
				"fileDesc" : "图片文件",
		...
		var uploadObj = bindingUpload(config,function(event,queueID,fileObj,response,data) {
		    if(response=="error"){
		        alert("文件上传失败，请重试！")
		        	...
		        	if(data.fileCount<=0){
			        	alert("文件上传完毕！");
			        	location.href="${pageContext.request.contextPath}/chatGroup/show/${currentChatGroup.id}.htm";
			        	}
		            }
		    },{"entityId":"${currentChatGroup.id}"});
		});
</script>
</head>
...
<div class="gxz">
    ...
    <div class="GTOut_rightStepDiv">
    	...
    	<a href="${pageContext.request.contextPath}/chatGroup/update/${currentChatGroup.id}.htm" class="GTOut_rightTab ">基本信息</a><b class="GTOut_rightTabLine">|</b>
        <a href="javascript:;" class="GTOut_rightTab GTOut_rightTabNow">修改头像</a>
    </div>
    </div>
    <form>
    <p class="GT_photo100"><a href="javascript:;"><img src="${pageContext.request.contextPath}/${currentChatGroup.avatar.showPath}" /></a></p>
