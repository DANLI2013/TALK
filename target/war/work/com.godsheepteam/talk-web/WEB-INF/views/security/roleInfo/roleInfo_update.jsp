		...
		<div id="gxz">
			...
			<spring:bind path="roleInfo.authority">
				...
				<li>
					...
					tree.N['<c:choose><c:when test="${menu.parentMenu==null}">1</c:when><c:otherwise>${menu.parentMenu.id}</c:otherwise></c:choose>_${menu.id}'] = 'ctrl:menuIds;checked:<c:choose><c:when test="${sf:contains(roleInfo.menus,menu)}">1</c:when><c:otherwise>0</c:otherwise></c:choose>;T:${menu.menuName}';
					</c:forEach>
					tree.wordLine = false;
