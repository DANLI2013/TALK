...
<title><%=application.getServletContextName()%>——上传新群组头像</title>
...
<script type="text/javascript">
	$(function(){
		var config = {
				...
				"url" : "${pageContext.request.contextPath}/buildChatGroupApp/uploadAppCover.htm;jsessionid=<%=session.getId()%>",
				"multi" : false,
				"num" : 1,
				"size" : 204800,
				"fileDesc" : "图片文件",
		...
		var uploadObj = bindingUpload(config,function(event,queueID,fileObj,response,data) {
		    if(response=="error"){
		        alert("文件上传失败，请重试！")
		        	...
		        	if(data.fileCount<=0){
			        	alert("文件上传完毕！");
			        	location.href="${pageContext.request.contextPath}/buildChatGroupApp/finish.htm";
			        	}
		            }
		    },{appId:"${appId}"});
		});
...
<div class="gxz">
    ...
    <form>
    <p class="GT_photo100"><a href="javascript:;"><img src="${pageContext.request.contextPath}/wd/img/gstTalk_out04.gif" /></a></p>
